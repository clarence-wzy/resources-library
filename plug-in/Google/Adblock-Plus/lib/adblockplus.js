  ...
  var Notification = exports.Notification = {
    ...
    getNextToShow: function(url)
    {
      ...
      for (var _loopIndex35 = 0; _loopIndex35 < notifications.length; ++_loopIndex35)
      {
        ...
        if ((typeof notification.type === "undefined" || notification.type !== "critical") && Prefs.notificationdata.shown.indexOf(notification.id) !== -1)
        {
          continue;
        }
    ...
    getLocalizedTexts: function(notification, locale)
    {
      ...
      var localizedTexts = [];
      for (var _loopIndex38 = 0; _loopIndex38 < textKeys.length; ++_loopIndex38)
      {
        var key = textKeys[_loopIndex38];
    ...
    triggerQuestionListeners: function(id, approved)
    {
      ...
      var questionListeners = listeners[id];
      for (var _loopIndex39 = 0; _loopIndex39 < questionListeners.length; ++_loopIndex39)
      {
        var listener = questionListeners[_loopIndex39];
        listener(approved);
      }
    }
  };
...
require.scopes["antiadblockInit"] = (function()
{
  ...
  exports.initAntiAdblockNotification = function initAntiAdblockNotification()
  {
    ...
    {
      var urlFilters = [];
      for (var _loopIndex40 = 0; _loopIndex40 < subscription.filters.length; ++_loopIndex40)
      {
        var filter = subscription.filters[_loopIndex40];
        if (filter instanceof ActiveFilter)
        {
          for (var domain in filter.domains)
          {
            var urlFilter = "||" + domain + "^";
  ...
  window.URL = (function()
  {
    ...
    }))
    {
      ...
      URL = function(url, baseUrl)
      {
        ...
        anchor.href = url;
        for (var _loopIndex41 = 0; _loopIndex41 < URLProperties.length; ++_loopIndex41)
        {
          var prop = URLProperties[_loopIndex41];
...
require.scopes["stats"] = (function()
{
  ...
  var statsPerPage = new ext.PageMap();
  var getStats = exports.getStats = function getStats(key, page)
  {
    if (!page)
    {
      return key in Prefs.stats_total ? Prefs.stats_total[key] : 0;
    }
    var pageStats = statsPerPage.get(page);
...
require.scopes["whitelisting"] = (function()
{
  ...
  var _tempVar42 = require("url");
  var stringifyURL = _tempVar42.stringifyURL;
  var getDecodedHostname = _tempVar42.getDecodedHostname;
  var extractHostFromFrame = _tempVar42.extractHostFromFrame;
  var isThirdParty = _tempVar42.isThirdParty;
  var pagesWithKey = new ext.PageMap();

  function isPageWhitelisted(page)
  {
    var url = page.url;
    var filter = defaultMatcher.matchesAny(stringifyURL(url), "DOCUMENT", getDecodedHostname(url), false, null);
  ...
  function isFrameWhitelisted(page, frame, type)
  {
    while (frame)
    {
      ...
      var filter = defaultMatcher.matchesAny(stringifyURL(url), type || "DOCUMENT", documentHost, isThirdParty(url, documentHost), getKey(page, frame));
      if (filter instanceof WhitelistFilter)
      {
        return true;
      }
  ...
  function processKey(token, page, frame)
  {
    ...
    var _tempVar43 = token.split("_", 2);
    var key = _tempVar43[0];
    var signature = _tempVar43[1];
    key = key.replace(/=/g, "");
...
require.scopes["filterValidation"] = (function()
{
  ...
  var InvalidFilter = _tempVar44.InvalidFilter;
  var ElemHideBase = _tempVar44.ElemHideBase;
...
{
  var exports = {};
  var _tempVar45 = require("url");
  var extractHostFromFrame = _tempVar45.extractHostFromFrame;
  var stringifyURL = _tempVar45.stringifyURL;
  var isThirdParty = _tempVar45.isThirdParty;
  var _tempVar46 = require("whitelisting");
  var getKey = _tempVar46.getKey;
  var isFrameWhitelisted = _tempVar46.isFrameWhitelisted;
  var defaultMatcher = require("matcher").defaultMatcher;
  var WhitelistFilter = require("filterClasses").WhitelistFilter;
  ...
  function composeFilters(details)
  {
    ...
    if (!isFrameWhitelisted(page, frame, "DOCUMENT"))
    {
      ...
      for (var _loopIndex47 = 0; _loopIndex47 < details.urls.length; ++_loopIndex47)
      {
        var url = details.urls[_loopIndex47];
        var urlObj = new URL(url, details.baseURL);
        if (urlObj.protocol == "http:" || urlObj.protocol == "https:")
        {
          url = stringifyURL(urlObj);
          var filter = defaultMatcher.matchesAny(url, details.type, docDomain, isThirdParty(urlObj, docDomain), getKey(page, frame));
          if (!(filter instanceof WhitelistFilter))
          {
      ...
      if (filters.length == 0 && !isFrameWhitelisted(page, frame, "ELEMHIDE"))
      {
        ...
        for (var _loopIndex48 = 0; _loopIndex48 < selectors.length; ++_loopIndex48)
        {
          var selector = selectors[_loopIndex48];
